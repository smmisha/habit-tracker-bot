...
class BusinessConnectionClient:
    ...
    """
    async def send_message_to_partner(self, business_connection_id: str, partner_username: str, text: str, bot=None) -> bool:
        ...
        if not partner_username or not str(partner_username).strip():
            logger.warning("Попытка отправки сообщения без partner_username")
            return False

        target_chat = None
        try:
            username = str(partner_username).replace("@", "").strip()
